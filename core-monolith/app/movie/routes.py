...
@movie_router.patch("/admin/content/{id}/status", response_model=MovieSummaryResponse)
async def update_movie_content_status(
    id: UUID,
    body: ContentStatusUpdateRequest,
    admin_user: AuthUserDomain = Depends(require_role("ADMIN")),
    movie_service: MovieService = Depends(get_movie_service),
):
    try:
        dto = await movie_service.update_movie_status(movie_id=id, new_status=body.status)
        return MovieSummaryResponse(
            id=dto.id,
            title=dto.title,
            original_title=dto.original_title,
...
from app.movie.schemas import CreateVenueRequest

@movie_router.post("/partner/venues", status_code=status.HTTP_201_CREATED)
async def create_partner_venue(
    body: CreateVenueRequest,
    partner: Partner = Depends(required_approved_partner),
    movie_service: MovieService = Depends(get_movie_service),
):
    venue = await movie_service.create_venue(
        name=body.name,
        city=body.city,
        partner_id=partner.id,
    ...
    return {"venue_id": venue.id, "name": venue.name, "status": "created"}

@movie_router.put("/partner/screens/{id}/layout")
async def put_screen_layout(
    id: UUID,
    body: ApplyLayoutRequest,
    partner: Partner = Depends(required_approved_partner),
    movie_service: MovieService = Depends(get_movie_service),
